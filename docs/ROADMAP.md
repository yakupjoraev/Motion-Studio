...
**Carried out of M4 and closed in M7:** the animated WebM hover clip for the block palette. The owner
chose the ffmpeg route in prompt 37; the clips are generated by reconstructing each animation frame by
frame rather than recording it, which makes them byte-identical across runs like the stills.
ADR-182 has the measurements.
