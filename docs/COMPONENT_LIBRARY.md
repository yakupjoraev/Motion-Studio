...
Each block ships a static WebP thumbnail (`320 × 200`, generated by a Playwright script that
renders `previewProps` in both light and dark themes and screenshots it) plus an animated
variant played on hover — a short WebM, and only when the user is not in reduced motion.

`pnpm generate:thumbnails` regenerates them; CI verifies every registry entry has a current
thumbnail, so a block cannot be added without one.
