...
| --- | --- | --- | --- |
| First-load JS | ≤ 250 kB gzip | **250.4 KiB — 430 B over** once the route's own chunk was measured again (ADR-371) | `size-limit` |
...
| --- | --- | --- | --- |
| `landing first-load JS` | every chunk `/` loads | 120 KiB | 107.8 KiB |
| `studio first-load JS` | every chunk `/studio` loads | 250 KiB | **250.4 KiB — over by 430 B** |
| `playground route chunk` | the chunks only `/playground` loads | 90 KiB | 45.1 KiB |
| `blocks route chunk` | the chunks only `/blocks` loads | 140 KiB | 12.9 KiB |

The measured column is a local build taken on 2026-09-06, and it is the first one in which the
numbers are complete: until ADR-371 the config's glob could not match a route's own chunk — the path
carries `[locale]` — so three of these four entries were reported light, the studio by 28.5 kB. The
studio is over its budget and the gate now says so; ADR-371 states why the threshold does not move to
meet it.
